...
class OneCallWeatherManager: ObservableObject {
    ...
    private let lat = 6.150701
    private let lon = 80.694771
    ...
    func requestForecast(url: String) async {
        ...
        do {
            ...
            let weather =  try JSONDecoder().decode(OnCallWeather.self, from: data)
            ...
            DispatchQueue.main.async {
                ...
                let current = OCWeatherDisplayHourly(dt: first.dt.unixToDate(date: .omitted, time: .shortened)!,
                ...
                self.weather = OCWeatherModel(forecast: forecasts, hourlyForecasts: hourly, current: current)
            }
